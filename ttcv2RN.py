...
class Pokemon:
    ...
    def __init__(self, number, name, form, type1, type2, moveset, hp, attack, defense, sp_attack, sp_defense, speed, nature, battle_rules, item=None, *args, **kwargs):
        self.number = number
        self.name = name
        self.form = form
        self.type1 = type1
    ...
    def add_pokemon(self, pokemon_name):
        ...
        if new_pokemon_info:
            ...
            if new_pokemon.is_banned():
                print(f"{new_pokemon.name} is banned and cannot be added to the team.")
                return
            if not new_pokemon.check_play_restrictions(self.pokemons):
                print(f"Failed to add {new_pokemon.name} to the team due to play restrictions.")
                return
